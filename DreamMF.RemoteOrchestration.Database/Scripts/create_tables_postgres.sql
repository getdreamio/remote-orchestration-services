-- PostgreSQL script to create tables based on the entities
...
CREATE TABLE AuditReads_Host (
    AuditRead_ID SERIAL PRIMARY KEY,
...
CREATE TABLE Remote_Module (
    Remote_Module_ID SERIAL PRIMARY KEY,
    Remote_ID INTEGER NOT NULL,
...
CREATE TABLE Host_Remote (
    Host_Remote_ID SERIAL PRIMARY KEY,
    Host_ID INTEGER,
    Remote_ID INTEGER,
    Created_Date BIGINT NOT NULL,
    Updated_Date BIGINT NOT NULL,
...
CREATE TABLE Tags_Host (
    Tag_Host_ID SERIAL PRIMARY KEY,
    Host_ID INTEGER,
    Tag_ID INTEGER,
...
CREATE TABLE Tags_Remote (
    Tag_Remote_ID SERIAL PRIMARY KEY,
    Remote_ID INTEGER,
    Tag_ID INTEGER,
    Value VARCHAR(500) NOT NULL,
    Created_Date BIGINT NOT NULL,
...
VALUES 
    ...
    -- Database Settings
    ('database:type', 'postgres', 1671645260, 1671645260),
...
SELECT 
    ...
    COUNT(CASE 
        WHEN ar.Created_Date >= EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - INTERVAL '30 days'))::bigint 
        THEN 1 
...
SELECT 
    to_timestamp(ar.Created_Date)::date as ReadDate,
    h.Host_ID,
    h.Name as HostName,
    COUNT(*) as TotalReads,
...
FROM Host h
LEFT JOIN AuditReads_Host ar ON h.Host_ID = ar.Host_ID
GROUP BY to_timestamp(ar.Created_Date)::date, h.Host_ID, h.Name
...
FROM Remote r
LEFT JOIN AuditReads_Remote ar ON r.Remote_ID = ar.Remote_ID
GROUP BY to_timestamp(ar.Created_Date)::date, r.Remote_ID, r.Name
ORDER BY ReadDate DESC;
