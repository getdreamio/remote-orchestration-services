...
export const TagInput: React.FC<TagInputProps> = ({
	...
	const ifDisplayNameSame = (tag: Tag) =>
		tag.display_Name.trim().toLowerCase() === tag.key.trim().toLowerCase();
		...
		<div className="space-y-2">
			<div className="flex flex-wrap gap-2">
				{tags?.map((tag) => {
					...
					return (
						<Tooltip
							key={`${tag.key}:${tag.display_Name}`}
							title={`${tag.key}: ${tag.display_Name}`}
						>
							{tagElement}
						</Tooltip>
					);
				...
				<div className="flex gap-2 items-start">
					<div className="flex-1">
						<Select
                            ...
                            options={allTags?.map((tag) => {return{
                                value: tag.tag_ID,
                                label: ifDisplayNameSame(tag)
                                    ? tag.key
                                    : `${tag.key}:${tag.display_Name}`,
                            }})}
						>
						</Select>
					</div>
